...
class HTTPStatusCodes(Enum):
    """An Enum containing the HTTP Status Code Names with their corresponding
    HTTP Status Codes."""

    CONTINUE = 100
    """This interim response indicated that the client should continue the
    ...
    PROCESSING = 102
    """[DEPRECATED] This code was used in WebDAV contexts to indicate that a
    request has been received by the server, but no status was available at the
    ...
    letting the user agent start preloading resources while the server prepares
     a response or preconnect to an origin from which the page will need
     resources."""
...
"""
    ...
    """The request has been received but not yet acted upon. It is
     noncommittal, since there is no way in HTTP to later send an asynchronous
      response indicating the outcome of the request. It is intended for cases
      where another process or server handles the request, or for batch
      processing."""
    ...
    NO_CONTENT = 204
    """There is no content to send for this request, but the headers are
    ...
    possible response and the user agent or user should choose one of them.
    There is no standardized way for clients to automatically choose one of
    the responses, so this is rarely used."""
    ...
    USE_PROXY = 305
    """[DEPRECATED] Defined in a previous version of the HTTP specification to
    ...
    specified by the Location response header. This has the same semantics as
    the 301 Moved Permanently HTTP response code, with the exception that the
    ...
    however this status code is rarely used and no standard convention
    exists."""
    ...
    send this response instead of 403 Forbidden to hide the existence of a
    ...
    server-driven content negotiation, doesn't find any content that conforms
    ...
    since some browsers use HTTP pre-connection mechanisms to speed up
    browsing. Some servers may shut down a connection without sending this
    message."""
    ...
    """The ranges specified by the `Range` header field in the request cannot
    be fulfilled. It's possible that the range is outside the size of the
    target resource's data."""
    ...
    TOO_EARLY = 425
    """[EXPERIMENTAL] Indicates that the server is unwilling to risk processing
    ...
    is intended to prevent the 'lost update' problem, where a client `GET`s a
    ...
    should be sent. This response should be used for temporary conditions
    and the `Retry-After` HTTP header should, if possible, contain the
    estimated time before the recovery of the service. The webmaster must also
    take care about the caching-related headers that are sent along with this
